/* ============================================================
   ...
   este producto, subidos a Vercel Blob (privado, NO al repo — el
   repo es público). Cada entrada es { nombre, blobUrl }. Súbelos
   con el script scripts/subir-archivo-toolkit.mjs y pega aquí la
   URL que te devuelva.
...
export const TOOLKIT_PRODUCTOS = [
  ...
  },
  {
    ...
    archivos: [{ nombre: "power-bi-de-cero-a-hero.pdf", blobUrl: "" }],
    detalle: {
  ...
  },
  {
    ...
    archivos: [
      { nombre: "dataset-punto-de-venta.zip", blobUrl: "" },
      { nombre: "recetario.sql", blobUrl: "" },
      { nombre: "guia-recetario.pdf", blobUrl: "" },
      { nombre: "fundamentos.pdf", blobUrl: "" },
    ],
    detalle: {
...
/** Busca la URL (privada, en Vercel Blob) de un archivo por nombre,
 *  pero SOLO entre los productos que la orden realmente cubre
 *  (`productIds`) — así un token válido para un producto no puede
 *  usarse para pedir el archivo de otro. */
export function getArchivoBlobUrl(productIds, nombreArchivo) {
  for (const productoId of productIds) {
    const producto = TOOLKIT_PRODUCTOS.find((p) => p.id === productoId);
    const archivo = producto?.archivos?.find((a) => a.nombre === nombreArchivo);
    if (archivo) return archivo.blobUrl || null;
  }
  return null;
}
