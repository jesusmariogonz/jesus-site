...
export async function sendKindleCopy({ titulo, resumen, url, contenidoHtml }) {
  ...
  // resumen) para que se pueda leer entera en el Kindle.
  const documento = `
    <html>
      <head><meta charset="utf-8" /><title>${titulo}</title></head>
      <body>
        <h1>${titulo}</h1>
        ${resumen ? `<p><em>${resumen}</em></p>` : ""}
        ${contenidoHtml || ""}
        <p>Ver en el sitio: <a href="${url}">${url}</a></p>
      </body>
    </html>
  `;
